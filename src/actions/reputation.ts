...
import { computePotentialRpGain } from "@/lib/reputation/potential-rp";
...
export async function getPotentialRpGain(taskId: string, userId: string): Promise<number> {
    ...
    try {
        ...
        if (ownedResult.error || vouchedResult.error) return 0;
        ...
        return computePotentialRpGain([...owned, ...vouched], taskId, userId);
    } catch {
        return 0;
    }
}
