...
- A **deadline**
- A **failure cost** (in EUR/USD/INR, stored in cents) — charged to a charity if the task is missed
- A **voucher** — a friend (or themselves) who approves/denies completion claims

The core loop: commit to something, get a friend to vouch for it, complete it before the deadline, let the voucher confirm it. Miss it → pay up.
...
| State Machine | XState v5 (advisory only — DB writes happen in server actions) |
| Confetti | `canvas-confetti` via `src/lib/confetti.ts` — `fireCompletionConfetti()` fired on task completion from both dashboard and task detail |
| Calendar Integration | Google Calendar API (bidirectional OAuth sync) |
| Hosting | Vercel |
| Mobile | Capacitor (iOS/Android — wrappers over the web app) |
...
src/
  ...
  trigger/          # Background jobs (Trigger.dev)
    deadline-fail.ts        # Every 5 min — fail overdue tasks
    voucher-timeout.ts      # Hourly — auto-accept + penalize voucher
    recurrence-generator.ts # Scheduled — generate next recurring task
    ledger-settlement.ts    # Monthly — send settlement emails
    task-reminder-notify.ts # Every 1 min — push/email reminders
    task-proof-cleanup.ts   # Periodic — purge stuck/stale proofs
...
- `voucher_can_view_active_tasks` — whether voucher can see owner's active tasks
...
- `recurrence_rule_id` (nullable FK → `recurrence_rules`)
- `requires_proof` (bool), `has_proof` (bool — denormalized, set on accept)
...
### `friendships`
Directed edges. Two rows per friendship (A→B and B→A). FK to `profiles`.
...
Outbox queue for app→Google sync operations.
...
```
CREATED ──────────────────────────────────────────────► DELETED (voucher or owner <5min)
   │
   ├─► POSTPONED (once, before deadline) ──────────────► DELETED (voucher)
   │         │
   └─────────┴─► AWAITING_VOUCHER (owner marks complete)
                        │
              ┌─────────┴─────────┐
              ▼                   ▼
          COMPLETED           FAILED
         (accept or          (deny or
          timeout)            missed deadline)
              │                   │
              │              RECTIFIED (voucher authorizes, 7-day window, ≤5/month)
              │                   │
              └────────┬──────────┘
                       ▼
                   SETTLED (month-close email — tasks stay in DB state)
```

**Key rules:**
- Self-vouched tasks skip AWAITING_VOUCHER → go directly to COMPLETED
- Voucher timeout auto-accepts (→ COMPLETED) and charges voucher 30 cents, not the owner
...
**Scale**: 0–1000. New users start around 400 (Bayesian smoothing with weight=20).
...
|---|---|---|
| Delivery | 35% | `completed_with_timestamp / finalized_tasks × 1000` |
| Accountability | 20% | Starts 1000, −80 per FAILED (×decay×consecutive mult), −25 per postpone |
| Community | 10% | Starts 1000, +15 per task vouched for others, −30 per timeout auto-accept |

**Bonus points** (additive, never penalize — only rewarded if features are used):
| Bonus | Cap | How |
|---|---|---|
| Discipline | +75 pts | Streak multipliers on recurring tasks (×1.3 @7d, ×1.6 @14d, ×2.0 @30d) |
| Proof Quality | +50 pts | `tasks_with_proof / eligible_completed_non-selfvouch × 1000` |
| Pomo | +50 pts | 1 pt per 10 min of pomo on completed tasks |

**Bayesian smoothing**: `(20 × 400 + taskCount × rawScore) / (20 + taskCount)`

**Velocity delta**: score now vs score 7 days ago (shown as ↑/↓ on the bar)
...
`src/components/ReputationBar.tsx` — renders score/1000 as an orange gradient progress bar with velocity delta label. No tier label shown (just the number).

### Potential RP (pre-completion preview)
`src/lib/reputation/potential-rp.ts` — `computePotentialRpGain(tasks, taskId, userId)` pure function. Runs algorithm twice (current vs simulated COMPLETED), returns positive delta or 0.
...
- App bounds: EUR/USD 1.00–100.00, INR 50–1000. DB hard bounds: 1–100000 cents.
- Ledger entry types: `failure` (+, owner charged), `rectified` (−, cancels failure), `voucher_timeout_penalty` (+, voucher charged 30¢), `force_majeure` (−, not yet implemented in UI)
...
| `tasks.ts` | `undoTaskComplete` | Revert AWAITING_VOUCHER → CREATED/POSTPONED (while deadline in future) |
| `voucher.ts` | `voucherAccept` | AWAITING_VOUCHER → COMPLETED |
| `voucher.ts` | `voucherDeny` | AWAITING_VOUCHER → FAILED + ledger entry |
| `voucher.ts` | `authorizeRectify` | FAILED → RECTIFIED + negative ledger entry |
...
| `voucher-timeout` | Hourly | Auto-accept AWAITING_VOUCHER past voucher deadline; charge voucher 30¢ |
...
- **Realtime**: Supabase subscriptions on `tasks`, `friendships`, `pomo_sessions`. Client emits `CustomEvent<RealtimeTaskChange>` on `window`. Components listen via `subscribeRealtimeTaskChanges`. Stale-reconcile lock prevents background job writes reverting optimistic state.
- **Cache tags**:
  - `tasks:active:{userId}` — TTL 60s, invalidated on task status change
  - `voucher:pending:{voucherId}` — TTL 300s, invalidated on voucher actions

---

## 11. Task Title Parser Tokens

`src/lib/task-title-parser.ts` — parses inline tokens from task title:
...
- **VFD Pomodoro color**: `text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.6)]` — used for the timer display and the "Time Focused" stat.
- **Stat metric glows**: all stat numbers on stats and ledger pages use `drop-shadow` glows matching their text color. Color→glow map:
  - white → `rgba(255,255,255,0.4)` (unused now, Active uses blue-400)
  - blue-400 (Active) → `rgba(96,165,250,0.6)`
  - cyan-400 → `rgba(34,211,238,0.6)`
  - purple-400 → `rgba(192,132,252,0.6)`
  - lime-300 → `rgba(190,242,100,0.6)`
  - red-500 → `rgba(239,68,68,0.6)`
  - pink-500 → `rgba(236,72,153,0.6)`
  - orange-400 → `rgba(251,146,60,0.6)`
  - green-400 → `rgba(74,222,128,0.6)`
...
- **Confetti**: `fireCompletionConfetti()` from `src/lib/confetti.ts` (uses `canvas-confetti`) fires when a task is marked complete — called in both `dashboard-client.tsx` and `task-detail-client.tsx`.
...
1. **Force majeure** — schema, types, and state machine support it, but no server action creates it. UI entry point doesn't exist.
2. **`SETTLED` status** — monthly email references it, but tasks are never DB-updated to SETTLED. Tasks stay in COMPLETED/FAILED/RECTIFIED permanently.
3. **Voucher response deadline** — XState says +7 days; actual server action (`getVoucherResponseDeadlineUtc`) is end-of-day + 2 days in user timezone. Use server action as truth.
4. **`has_proof` flag** — set by `voucherAccept` when proof cleanup happens; used by reputation's proof quality score. Self-vouched tasks never set it.
5. **Weekly recurrence with `interval > 1`** — not correctly implemented; falls through to always-run.
6. **`@ts-ignore` and `as any` casts** — `types.ts` wasn't regenerated after schema migrations. Expect TypeScript to complain on new Supabase queries.
